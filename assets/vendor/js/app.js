require('../css/jquery.countdown.css');
...
(function($){
  ...
  $(window).on('load', function(){
    ...
    window.initCountdown = function () {
      let battleEndDate = $("#current-battle-remaining").attr('data-end-date');
      let battleEndDateFormatted = moment(battleEndDate, 'YYYY-MM-DD HH:mm:ss').format('MM/DD/YYYY HH:mm:ss');

      if ($('#current-battle-remaining').length > 0) {
        $('#current-battle-remaining').countdown({
          date: battleEndDateFormatted,
          offset: +2, //@todo add gmt from backend
          day: 'Day',
          days: 'Days',
          hideOnComplete: false
        }, function (container) {
          // todo: do something when finished! Maybe refresh page?
        });
      }
    }
